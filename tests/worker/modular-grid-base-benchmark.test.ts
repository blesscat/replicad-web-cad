...
import {
  ...
  modularGridBaseTemplateUrl,
} from '../../src/cad-kernel/components/modular-grid-base/builder'
import {
  createDefaultModularGridBenchmarkAdapter,
...
const TEMPLATE_PATH = modularGridBaseTemplateUrl
...
const selectedFixtures = process.env.RUN_CAD_BENCHMARK_FIXTURES
  ? MODULAR_GRID_BENCHMARK_FIXTURES.filter((fixture) =>
      process.env.RUN_CAD_BENCHMARK_FIXTURES?.split(',').includes(
        `${fixture.rows}x${fixture.columns}`,
      ),
    )
  : undefined
const selectedStrategies: Array<'sequential' | 'balanced'> | undefined = process
  .env.RUN_CAD_BENCHMARK_STRATEGIES
  ? process.env.RUN_CAD_BENCHMARK_STRATEGIES.split(',').filter(
      (strategy): strategy is 'sequential' | 'balanced' =>
        strategy === 'sequential' || strategy === 'balanced',
    )
  : undefined
...
describe.skipIf(!runBenchmark)('modular-grid-base Worker benchmark', () => {
  beforeAll(async () => {
  ...
  it('reports phase timings and enforces the performance gate', async () => {
    const report = await runModularGridBenchmark({
      adapter: {
        ...createDefaultModularGridBenchmarkAdapter(loadTemplate),
    ...
    expect(report.runs.every((run) => run.quality.meshTriangleCount > 0)).toBe(
      true,
    )
    expect(
