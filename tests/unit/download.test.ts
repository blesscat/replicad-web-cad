...
import { PROTOCOL_VERSION, type ExportReadyEvent } from "../../src/cad-contract/messages";
import { validateStepResponse } from "../../src/features/cad/download";

function stepResponse(overrides: Record<string, unknown> = {}): ExportReadyEvent {
  return {
...
describe("STEP response validation", () => {
  it("accepts a non-empty response for the expected revision", () => {
    expect(validateStepResponse(stepResponse(), "rev-1", "epoch-1", "box-20x30x40.step")).toEqual({ valid: true });
  });
  ...
  it.each(invalidCases)("rejects %s", (overrides) => {
    expect(validateStepResponse(stepResponse(overrides), "rev-1", "epoch-1", "box-20x30x40.step").valid).toBe(false);
  });
});
