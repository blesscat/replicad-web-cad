...
import { RevisionLifetime, type CandidateRecord } from "../../src/cad-kernel/lifetime";
import type { MeshData } from "../../src/cad-kernel/mesh";

function candidate(id: string, generation: number, shape: Solid): CandidateRecord {
  const mesh: MeshData = {
...
describe("revision lifetime", () => {
  it("evicts replaced candidates at the configured capacity", () => {
    ...
    expect(store.pendingCandidates.map((item) => item.candidateId)).toEqual(["c2", "c3"]);
  });
