...
import { boundsForBox, type BoxBounds, type BoxParameters } from "../../src/cad-contract/units";
import {
...
function annotationMap(annotations: DimensionAnnotation[]): Map<DimensionAnnotation["key"], DimensionAnnotation> {
...
describe("CAD dimension annotation geometry", () => {
  it("creates readable X, Y and Z annotations for the prototype box", () => {
    ...
    expect(annotations.map((annotation) => annotation.key)).toEqual(["width", "depth", "height"]);
    expect(width).toMatchObject({ axis: "X", value: 20, valueLabel: "20 mm", ariaLabel: "寬度 X 20 mm" });
    expect(depth).toMatchObject({ axis: "Y", value: 30, valueLabel: "30 mm", ariaLabel: "深度 Y 30 mm" });
    expect(height).toMatchObject({ axis: "Z", value: 40, valueLabel: "40 mm", ariaLabel: "高度 Z 40 mm" });
