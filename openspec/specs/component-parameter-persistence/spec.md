...
The versioned browser persistence MUST store valid pillar parameters under the
stable `opengrid-pillar` model id. Each entry MUST contain either the typed
fixed mode `{ mode: 'detachable-corner-seat' }`, or the typed `positioning`
mode with its integer `length` and numeric `offset`. The positioning offset
MUST remain within -0.5 through 0.5 mm at 0.05 mm increments. The OpenGrid
pillar entry MUST remain independent from every other component's parameter
entry. Removed `standard` and `thin-shell` modes MUST NOT be persisted as
current values.

#### Scenario: Restore saved pillar parameters

- **GIVEN** browser persistence contains `{ mode: 'positioning', length: 25,
  offset: 0.15 }` under `opengrid-pillar`
- **WHEN** the user opens `/cad/opengrid-pillar`
- **THEN** the positioning radio MUST be selected
- **AND** the total-length and XY increment controls MUST display the saved
  typed values
- **AND** the first generation MUST use the 25 mm positioning profile with
  the shared XY increment

#### Scenario: Restore the saved locking corner seat

- **GIVEN** browser persistence contains `{ mode: 'detachable-corner-seat' }`
  under `opengrid-pillar`
- **WHEN** the user opens `/cad/opengrid-pillar`
- **THEN** the locking corner-seat radio MUST be selected
- **AND** no positioning length or XY increment control MUST be active
- **AND** the first generation MUST use the fixed shared locking corner-seat
  geometry

#### Scenario: Persist a valid pillar update

- **GIVEN** a pillar snapshot with `mode=detachable-corner-seat`, or
  `mode=positioning` with a valid integer length and valid shared XY increment,
  passes validation
- **WHEN** the workspace accepts the update
- **THEN** persistence MUST update only the `opengrid-pillar` entry
- **AND** the stored value MUST remain the typed mode, positioning length when
  applicable, and numeric offset rather than raw input strings
...
- **WHEN** the user selects a removed or unsupported mode, or supplies a
  malformed, fractional-step, or out-of-range length or offset
- **THEN** the previous accepted `opengrid-pillar` entry MUST remain unchanged
- **AND** the invalid value MUST NOT be sent to the Worker as
  `model.generate`
...
- **THEN** it MUST use `{ mode: 'detachable-corner-seat' }`
- **AND** the invalid entry MUST NOT be sent to the Worker
- **AND** initialization MUST continue without treating persistence failure as
  a CAD failure

#### Scenario: Migrate old pillar snapshots

- **GIVEN** the persisted entry uses the old `{ length, baseConnection: false }`
  shape with a valid length
- **WHEN** the OpenGrid pillar workspace initializes
- **THEN** it MUST use `{ mode: 'positioning', length, offset: 0 }`
- **GIVEN** the persisted entry uses an old positioning shape with valid
  `offsetX` and `offsetY`
- **WHEN** those X/Y values are equal
- **THEN** it MUST normalize them to the shared positioning `offset`
- **WHEN** those X/Y values are unequal
- **THEN** it MUST preserve the valid positioning length and normalize the
  shared offset to `0`
- **GIVEN** the persisted entry uses `standard` or `thin-shell`, with or
  without legacy offsets
- **WHEN** the OpenGrid pillar workspace initializes
- **THEN** it MUST normalize the entry to
  `{ mode: 'detachable-corner-seat' }`
- **AND** old checkbox fields MUST NOT remain as active user parameters
