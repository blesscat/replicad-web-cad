...
OpenGrid 的 Desk 與 Wall 入口會對同一個 model 使用不同的初始 preset，但目前模型選擇頁與預覽 capture 只認得 model id，因此兩個入口無法同時保存互相隔離的參數，也無法產生反映各自 preset 的預覽圖。這會讓 Wall Snap 的卡片看起來像 Desk Snap，且 capture 不能驗證入口實際使用的初始設定。

## What Changes

- 新增穩定的 OpenGrid system context：`desk` 與 `wall`。
- 將 `/models` 的 OpenGrid 系列分成 `Desk System` 與 `Wall Related` 子分類。
- `底板` 與 `Snap` 同時出現在兩個子分類；其他目前的 OpenGrid 模型只出現在 Desk；HSW 維持單一入口。
- 子分類入口連結帶上 system context，例如 `/cad/opengrid-snap?system=desk` 與 `/cad/opengrid-snap?system=wall`。
...
- `opengrid-system-entry-context`: 定義 Desk／Wall context、入口解析、system-specific preset、scoped persistence 與 context preview capture identity。
