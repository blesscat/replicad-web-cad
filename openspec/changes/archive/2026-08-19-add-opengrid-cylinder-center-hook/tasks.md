...
- [x] 1.4 Add Worker/integration tests that exercise the centered rectangular hook, one-solid output, Full/Lite mating envelope, 90-degree capture relationship, and absence of outer locating geometry.
...
- [x] 3.1 Add fixed hook geometry constants for the nominal 8 × 4 interface, 0.2 mm per-side clearance, 3 mm downward span, and insertion orientation.
- [x] 3.2 Add the `center-hook` bottom-feature path to the cylinder Worker builder, fusing one centered rectangular hook at Z=0 and skipping stepped holes, round seats, and outer cardinal features.
...
- [x] 4.1 Extend cylinder quality inspection to recognize the hook's centered rectangular envelope, Z span, positive clearance, fused single-solid result, and lack of outer locating geometry.
