...
export class RevisionLifetime {
  ...
  get current(): RevisionRecord | null {
    return this.currentRevisionId ? this.revisions.get(this.currentRevisionId) ?? null : null;
  }
  ...
  addCandidate(candidate: CandidateRecord): CandidateRecord[] {
    ...
    while (this.candidates.size > this.candidateLimit) {
      const replaceable = [...this.candidates.values()]
        ...
        .sort((a, b) => a.generation - b.generation || a.createdAt - b.createdAt)[0];
    ...
    if (this.candidates.has(candidate.candidateId) && this.candidates.size > this.candidateLimit) {
  ...
  pruneCommitsBeforeGeneration(generation: number): void {
    for (const [operationId, commit] of this.commits) {
      if (commit.revision.generation < generation) this.commits.delete(operationId);
    }
  }
  ...
  commitCandidate(candidateId: string): RevisionRecord {
    ...
    if (previous && previous.modelRevision !== revision.modelRevision && previous.exportPins === 0) {
  ...
  cleanupExpired(latestGeneration: number, includeLatest = false): CandidateRecord[] {
  ...
  unpin(modelRevision: string): void {
    ...
    if (revision.modelRevision !== this.currentRevisionId && revision.exportPins === 0) {
  ...
  dispose(): void {
    for (const candidate of this.candidates.values()) deleteShape(candidate.shape);
