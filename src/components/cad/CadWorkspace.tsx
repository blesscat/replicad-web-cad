    ...
    >
      <div className="self-start grid gap-4 rounded-2xl border border-border-card bg-panel p-4">
          ...
          {boxDefinition.parameterSchema.map((field) => (
            <label className="grid gap-[0.3rem]" key={field.key}>
              ...
              {fieldErrors[field.key] && (
                <span className="text-sm text-error" id={`${field.key}-error`} role="alert">
                  {fieldErrors[field.key]}
                </span>
              )}
            </label>
        ...
        <div className="flex flex-wrap gap-[0.6rem]">
          <button className={ACTION_BUTTON_CLASS} type="button" disabled={!canExport} onClick={handleExport}>下載 STEP</button>
          {(state.status === "recoverable-error" || state.status === "fatal-worker-error") && (
            <button className={ACTION_BUTTON_CLASS} type="button" onClick={handleRetry}>重試</button>
          )}
        </div>
        <div aria-live="polite" className="rounded-2xl border border-border-card bg-panel p-4 text-[0.92rem] text-status" role="status">
          <strong className="text-ink">狀態：</strong> {status}
          {state.stale && <div className="text-muted">目前預覽是上一個成功 revision。</div>}
