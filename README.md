...
瀏覽器端 CAD Prototype：使用者調整 CAD component 的參數，以即時 3D 預覽查看結果，並從精確的 B-Rep 下載 STEP、STL，或受限的 Snap flat-text 3MF 檔案。
...
└─ Svelte workspace（components/cad/，主執行緒）
      ...
      └─ download adapter
         ...
         └─ STEP / binary STL / Snap 3MF export
```
...
- 匯出：由 Worker 目前 committed B-Rep 產生 STEP 或 binary STL；另有只適用於固定 Snap POC 組合的 multipart 3MF。
